...
if __name__ == "__main__":
    ...
    for idx, row in df_filtered.iterrows():
        ...
        if folder_type == 'react_ts':
            ...
            # 转换为file:// URL
            html_url = f"file://{index_html_path}"
            ...
            if not os.path.exists(release_dist_path):
                logging.warning(f"Folder not found: {release_dist_path}, skipping...")
                continue
...
当你获取了用户的代码，你可以参考js代码里的变量名称，写js代码来获取页面的信息；也可以结合提供的代码去判断应用实现的合理性、美观度等。"""
                ...
                if css_files:
                    for css_file in css_files:
                        ...
                        css_contents.append(f"======= {css_file_name} =======\n{css_content}")
                    all_css_content = "\n\n".join(css_contents)
                else:
                    all_css_content = ""
                ...
                # 读取所有JS文件内容并合并（js_files已经在flashapp_folders中排序）
                js_contents = []
                for js_file in js_files:
                    js_content = read_code_file(js_file)
                    js_file_name = os.path.basename(js_file)
                    js_contents.append(f"======= {js_file_name} =======\n{js_content}")
                
                all_js_content = "\n\n".join(js_contents)
                
                html_file_name = os.path.basename(html_file_path)
                
                # 拼接代码片段：CSS在前，然后是HTML，最后是JS
                # 格式模仿lingguang_run.py，每个部分都有清晰的分隔
                # 注意：all_css_content 和 all_js_content 已经包含了分隔符，不需要再添加
                if all_css_content:
                    code_snippet = f"""提供给你的css、html和js代码如下：
...
"""
                ...
                task_description = """对于每个任务，你会得到一个**用户需求(User Query)**、一个**网页地址(Target URL)**和页面的React/TypeScript代码（Code Snippet）。
当你获取了用户的代码，你可以参考代码里的变量名称和组件结构，写代码来获取页面的信息；也可以结合提供的代码去判断应用实现的合理性、美观度等。"""
