            ...
            for server_name, server_config in config_data.get("mcpServers", {}).items():
                ...
                if "url" in server_config:
                    self.mcp_servers[server_name] = {
                        ...
                        "tool_timeout": server_config.get("tool_timeout", 180.0)  # Default 3 minutes
                    }
                # Handle stdio server
        ...
        for action in actions:
            ...
            try:
                ...
                try:
                    ...
                    # Add timeout protection for MCP tool calls, especially for browser_run_code
                    # Default timeout: 180 seconds (3 minutes), can be overridden by server config
                    tool_timeout = self.mcp_servers.get(server_name, {}).get('tool_timeout', 180.0)
                    try:
                        result = await asyncio.wait_for(
                            server.call_tool(action_name, params),
                            timeout=tool_timeout
                        )
                    except asyncio.TimeoutError:
                        error_msg = f"Tool call '{action_name}' on server '{server_name}' timed out after {tool_timeout} seconds"
                        logger.error(error_msg)
                        action_result = ActionResult(
                            content=f"Error: {error_msg}. The operation may have taken too long or got stuck.",
                            keep=True
                        )
                    ...
                    # logger.info(f"调用工具 {server_name}__{action_name} 耗时: {duration:.2f} 秒")
                    # # 写到文件里
                    # try:
                    #     with open("mcp_tool_call_time.log", "a", encoding="utf-8") as f:
                    #         f.write(f"{server_name}__{action_name} # {duration:.2f} 秒\n")
                    # except Exception as log_file_error:
                    #     logger.error(f"Failed to write MCP tool call duration to file: {log_file_error}")
