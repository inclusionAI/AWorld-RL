...
class SequenceRunner(TaskRunner):
    ...
    async def _tool_call(self, policy: List[ActionModel], observations: List[Observation], step: int):
        ...
        for tool_name, action in tool_mapping.items():
            ...
            start_dt = datetime.fromtimestamp(tool_start_time)
            start_time_str = start_dt.strftime('%Y-%m-%d %H:%M:%S') + f".{int((tool_start_time - int(tool_start_time))*1000):03d}"
            ...
            end_dt = datetime.fromtimestamp(tool_end_time)
            end_time_str = end_dt.strftime('%Y-%m-%d %H:%M:%S') + f".{int((tool_end_time - int(tool_end_time))*1000):03d}"
            ...
            try:
                with open("time_log.txt", "a", encoding="utf-8") as f:
                    f.write(
                        ...
                        f"type: tool_use\n"
                        f"step: {str(step)}\n"
                        f"actions: {str(action)}\n"
                        f"开始时间: {start_time_str}\n"
                        f"结束时间: {end_time_str}\n"
                        f"耗时: {tool_duration:.4f}秒\n"
                        "-------------------------\n"
                    )
            except Exception as e:
                logger.warning(f"Failed to write tool time log: {e}")
