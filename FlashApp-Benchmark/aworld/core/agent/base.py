...
class Agent(BaseAgent[Observation, Union[List[ActionModel], None]]):
        ...
        if histories:
            ...
            if "tool_calls" in histories[-1].metadata and histories[-1].metadata['tool_calls']:
                ...
                if tool_id:
                    ...
                    try:
                        cur_msg['content'] = content.replace(histories[-1].content, "") # yingyu
                    except:
    ...
    async def async_policy(self, observation: Observation, info: Dict[str, Any] = {}, **kwargs) -> Union[
        ...
        query = getattr(self, "task", None)
        query_str = query if isinstance(query, str) else (getattr(query, "input", "") if query is not None else "")
        with open("time_log.txt", "a", encoding="utf-8") as f:
            f.write(
                f"query: {query_str}\n"
                f"type: async_desc_transform\n"
        ...
        def custom_serializer(obj):
            ...
            try:
                ...
                obj_dict["_type"] = type(obj).__name__
                return {key: custom_serializer(value) for key, value in obj_dict.items()}
            except AttributeError:
                raise TypeError(f"Object of type {type(obj)} is not JSON serializable")
        
        final_messages = {}
        final_messages["step"] = kwargs.get("step")
        final_messages["messages"] = messages
        final_messages["llm_response"] = llm_response
        with open("/Users/yuchengyue/AWorld_local/runs/闪应用采样_gemini_用户反馈200_dt0101-0108_mockapi_v2_修复static维度打分.json", "a") as f:
            f.write(json.dumps(final_messages, default=custom_serializer, ensure_ascii=False) + "\n")
