...
class OpenAIProvider(LLMProviderBase):
    ...
    def _init_async_provider(self):
        ...
        return AsyncOpenAI(
            ...
            base_url=base_url,
            timeout=self.kwargs.get("timeout", 180),
            max_retries=self.kwargs.get("max_retries", 10)
        )
    ...
    async def acompletion(self,
        ...
        try:
            # openai_params = self.get_openai_params(processed_messages, temperature, max_tokens, stop, **kwargs)
            openai_params = self.get_openai_params(processed_messages, temperature=1.0, max_tokens=64000, stop=stop, top_k=20, stream=False, **kwargs)
            import re
            print(len(re.findall(r"\b[a-zA-Z]+\b", str(openai_params["messages"]))))
            # if len(openai_params["messages"]) >= 3:
                # import json
                # with open("/Users/yuchengyue/AWorld_local/runs/playwright_content_show.txt", "a") as f:
                #     f.write(json.dumps(openai_params["messages"][3]["content"], ensure_ascii=False) + "\n\n\n")
            if self.is_http_provider:
                ...
                start_time_str = start_time_obj.strftime('%Y-%m-%d %H:%M:%S') + f".{int(start_time_obj.microsecond / 1000):03d}"
                response = await self.async_provider.chat.completions.create(**openai_params)
                end_time = time.time()
                end_time_obj = datetime.now()
                end_time_str = end_time_obj.strftime('%Y-%m-%d %H:%M:%S') + f".{int(end_time_obj.microsecond / 1000):03d}"
                duration = end_time - start_time
                try:
                    query = openai_params["messages"][1]["content"]
                except Exception:
                    query = ""
                step = len(openai_params["messages"])
