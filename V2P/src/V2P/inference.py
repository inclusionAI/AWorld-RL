...
# ---------------------------- 单区域拟合 ----------------------------
def _fit_single_region(normalized_centers,weights  
                       ) -> Tuple[float, float, float, float, bool]:
    """
    输入：一个连通区域的所有 (x, y, idx, score)
    输出：(mu_x, mu_y, sigma_x, sigma_y, success)
    """
    if len(normalized_centers) == 0:
        return 0.5, 0.5, np.nan, np.nan, False

    # 拆列，安全无越界
    x = np.array([item[0] for item in normalized_centers], dtype=float)
    y = np.array([item[1] for item in normalized_centers], dtype=float)
    w = np.array([item for item in weights], dtype=float) + 1e-12

    # 不足 3 点 → 直接回退
    if len(normalized_centers) <= 4:
        mu_x = np.average(x, weights=w)
        mu_y = np.average(y, weights=w)
        return mu_x, mu_y, np.nan, np.nan, False

    # 初值：加权平均 + 方差
    mu_x0 = np.average(x, weights=w)
    mu_y0 = np.average(y, weights=w)
    sig_x0 = max(np.sqrt(np.average((x - mu_x0)**2, weights=w)), 1e-3)
    sig_y0 = max(np.sqrt(np.average((y - mu_y0)**2, weights=w)), 1e-3)
    p0 = [mu_x0, mu_y0, sig_x0, sig_y0]

    def gauss(xy, mux, muy, sigx, sigy):
        xx, yy = xy
        return np.exp(-0.5 * (((xx - mux) / sigx)**2 + ((yy - muy) / sigy)**2))

    try:
        popt, _ = curve_fit(gauss, np.vstack([x, y]), w, p0=p0, maxfev=300)
        return *popt, True
    except RuntimeError:
        return *p0[:2], sig_x0, sig_y0, False
...
def get_prediction_region_point(attn_scores, n_width, n_height, top_n=30, activation_threshold=0.3, return_all_regions=True, rect_center=False):
    ...
    for region in regions:
        ...
        max_point = max(region, key=lambda item: item[3])
        # print(max_point)  # 输出: (11, 20, 101, 0.9)
        max_score = max(item[3] for item in region)
        region_scores.append(max_score)
        ...
        if not rect_center:
            # gauss average
            # print(region[0][:2])   # 期望输出类似 (0.23, 0.67))

            # mu_x, mu_y, _, _, success = _fit_single_region(
            #     normalized_centers=normalized_centers,   
            #     weights=weights          
            # )
            # avg_center_x, avg_center_y = mu_x, mu_y
