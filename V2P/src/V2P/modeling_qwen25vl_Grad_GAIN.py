...
from src.V2P.constants import IGNORE_INDEX
from typing import List, Tuple, Union, Optional
from src.V2P.trainer import rank0_print
...
class VisionHead_MultiPatch(nn.Module):
    ...
    def generate_grad_map(self, patch_logits, attn_weights, labels):
        gradient_maps = []

        for actor_index in range(labels.shape[0]):
            # 提取目标注意力权重和计算目标值
            target_attention_weights = attn_weights[actor_index][labels[actor_index] != 0 ]  # shape: (n_selected,)
            target_value = target_attention_weights.mean()

            # 使用 autograd.grad 计算目标值对 patch_logits 的梯度
            all_gradients = torch.autograd.grad(
                outputs=target_value,  # 目标值
                inputs=patch_logits,  # 需要计算梯度的参数
                retain_graph=True,  # 保留计算图，供后续使用
                create_graph=False  # 需要计算二阶导数
            )[0]  # 「第 i 个 <Actor> token 对图片每个 patch 的注意力」对「第 i 个 <Actor> token 对目标 patch 注意力的均值」 的影响力, shape: (n_visual,)
            gradients = all_gradients[actor_index]
            print(gradients)
            # 保存梯度
            gradient_maps.append(gradients)

        # 将梯度图转换为 Tensor
        gradient_maps = torch.stack(gradient_maps)  # shape: (n_target, n_visual)
        # gradient_maps = F.softmax(gradient_maps, dim=-1)  # 归一化梯度作为权重值

        # print(f"gradient_maps shape: {gradient_maps.shape}")
    ...
    def forward(self,
        ...
        # 获取梯度权重
        grad_map = self.generate_grad_map(patch_logits, attn_weights, labels)
        abs_grad_map = grad_map.abs()
        ...
        if (labels is not None) and (not do_single_patch):
            ...
            # reversed_labels = torch.zeros_like(labels)
            # reversed_labels[labels==0] = 1
            # print(f"labels: {labels}")
            # print(f"reversed_labels: {reversed_labels}")
            negative_attention = patch_logits * abs_grad_map
            negative_attention = F.softmax(negative_attention, dim=-1)
            negative_attention = negative_attention * reversed_labels
            negative_attn_sum = negative_attention.sum(dim=-1)
