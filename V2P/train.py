...
from V2P.dataset import LazySupervisedDataset
from V2P.trainer import AGUVISTrainer, rank0_print, safe_save_model_for_hf_trainer
from V2P.utils import dump_args_to_json
...
class DataArguments:
    ...
    label_style: Optional[str] = field(default="all") # gt label (all, center, gauss)
    sigma_factor: Optional[float] = field(default=4.1325)
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    print("-"*40)
    print(f"label_style is {data_args.label_style}.")
    print(f"sigma factor is {data_args.sigma_factor}.")
    print("-"*40)
    train_dataset = LazySupervisedDataset(
        tokenizer=tokenizer, processor=processor, data_path=data_args.data_path, label_style=data_args.label_style, sigma_factor=data_args.sigma_factor, data_args=data_args
    )
